...
colors:
  ...
  sea-glass: "#87d2c7"
  about-night: "#061a22"
  about-workbench: "#0d2933"
  about-paper: "#f3edda"
  about-muted: "#a9bbc0"
  about-brass: "#e2bc74"
  about-hairline: "#29444c"
typography:
  display:
...
展馆是航海日志系统中的独立沉浸表面。2026-09-23 根据用户下载的展品重新设计为航海藏品馆：海青色墙面、胡桃木结构、浅色石质展台、黄铜灯具与相框、奶油色大标题和纸质档案。新空间方向见 `docs/gallery-design/collection-concept.png`，取代原先满铺木材的前舱。以下规则仅约束 `/gallery`，保留本文件其余章节及全站 token；实施说明与素材来源见 [展馆设计说明](docs/gallery-design/museum.md)。

### 空间与容量

- 舱室宽约 10m；基础 6 个舱段按每段 3.4m 延展，随公开人物数量增长，最多支持 40 个展位。
- 当前运行时由 `MuseumRoom.ts` 构建封闭舱室、窄高双窗、中央 Logo 实墙、弧形肋木和地面罗盘。墙色 `#19353b`，展台 `#c4bca9` / `#d8d0bc`，凹进底座 `#172528`；这些是 3D 材料色，允许受场景照明影响。前舱以 `ArtifactDisplays.ts` 陈列六件主题道具：草帽圆台高 1.38m，和道一文字位于左舷纵向 0.55m 低台并向入口轻微偏转，六盏射灯替代中央吊灯，中央地毯保持迎宾动线。人物海报从 z=3.4 起布展，前方保持净空，由公开接口数据注入。
- 展位与画布保持稳定尺寸，最多维护邻近 12 张海报纹理；性能目标应以具体验收环境记录，不外推为通用 60fps 保证。

### 入口与导航

- 首次进入显示分区序厅：左侧标题、导览和“开始参观”，右侧以 16:9 完整显示真实场景截图 `collection-interior.webp`，人物展签和全屏按钮置于图像下方。序厅顶栏使用正常文档流；底部四章目录缩为紧凑导航，去除重复的大号章节计数。图像高度随视口预算缩放，禁止铺满页面裁切或与文案重叠；900px 以下改为文案、完整图像、章节导航的上下结构。所有文字与操作使用网页元素。漫游入口相机位置为 `(-0.35, 1.7, 2.8)`，宽屏视场角 58°，窄桌面自适应扩大到最多 78°以保留两侧展品净空。
- 桌面支持鼠标视角、WASD/方向键与 Pointer Lock 第一人称移动；E 或点击打开近距离展位，Escape 退出锁定。右上角使用半透明船舱小地图，显示船首/船尾、玩家方向和视野内展位。
- 小地图和准星属于展馆专属导航，不应抽象成全站通用组件。小屏和触摸设备保留视觉序厅，参观入口转为二维馆藏；章节目录可横向滚动，详情在窄屏改为单列。

### 字体与状态

展馆标题和策展文字使用本地 `Museum Serif`（Noto Serif SC），功能正文使用本地 `Museum Sans`（Noto Sans SC），年份与章节数字使用 Georgia 后备。字体限定在展馆，字重 400–700，按 Unicode 子集加载，源文件及 OFL 许可位于 `web/src/gallery/fonts`。局部深海底色为 `#061017`，黄铜为 `#c9a55c`，纸色为 `#eee4ce`；纸面详情使用深色文字和独立深色焦点描边。标题、按钮、暂停态与原生 `dialog` 均应保持键盘可用；减少动态偏好关闭入口位移动画。

### 验收边界

浏览器回归脚本 `scripts/gallery/check_browser.cjs` 默认把截图与报告写入系统临时目录 `gallery-museum-qa`。验收覆盖序厅、章节、检索、档案、漫游、容量与移动端降级；实时 Three.js 场景仍属于功能重建，几何细节、环境照明和材质质感与概念图存在差异，不能据此宣称严格 1:1 还原。

## `/about` 夜航工作台局部规则

2026-09-24 按用户选择的图 2「夜航工作台」及后续首屏越界参考升级。视觉依据为 `artifacts/about-design/2026-09-24/night/night.png`，当前实现采用分层工作台、可翻阅项目档案、工作航线、海岸生活场景和联系明信片。本页采用固定夜色，属于用户确认的独立沉浸表面；全站其余页面与系统主题规则保持各自语义。全站导航只适配夜色，不改变内容与既有交互；首屏下方的章节导航属于关于页内部。

### 布局与内容

- 首屏左侧展示身份、主张与操作，右侧为航船、罗盘、日志和完整圆台底座。海景仅在自身层内裁切，透明工作台单独定位，厚底越过首屏边界并压入章节导航上方预留的空间；不能把两层重新合并为一张受首屏裁切的背景。
- 内容最大宽度为 1320px。项目区采用左侧索引与右侧纸色档案，当前档案随索引、前后按钮或键盘切换，技术工具箱联动高亮匹配技能。长项目列表与单份详情分工，避免两列内容长度悬殊造成空洞；不添加没有数据依据的项目封面、技能百分比或成就数。
- 个人简介与指标置于独立手记面板；工作经历采用左侧停驻标题与右侧连续航线；生活区以海岸场景承接兴趣和紧凑所在地地图；本站说明保持文字阅读；联系区用邮箱、社交入口和可翻到二维码的明信片收尾。
- 模块继续读取既有公开接口，项目、经历、指标、兴趣、技术标签和社交入口数量均由数据决定，长文案允许自然换行。空内容显示整理状态或按模块条件隐藏；没有简历时不显示简历操作。请求失败提供重试，不以本地示例资料填充线上页面。
- 1199px、900px 和 650px 逐级调整留白与分栏。手机首屏按文案实际高度排版，预留独立船体空间；项目索引转为横向滚动，选中项自动移入索引可视范围，详情在下方完整阅读；个人手记、经历和联系逐步转为单列。

### 字体、颜色与图像

- 本页使用 `about-night` 环境、`about-workbench` 工作面、`about-paper` 主文字、`about-muted` 辅助字和 `about-brass` 操作反馈。项目档案和明信片使用 `#e8e0ca` 纸面与深墨文字，形成阅读节奏；黄铜用于航线、选中和操作，保持少量。
- 主标题沿用全站 `--display-font`；正文使用 Noto Sans SC / Microsoft YaHei，日期与技术读数使用 IBM Plex Mono。标题随视口缩放，正文与技术标签分别保证长文阅读和扫描；装饰性英文配有中文说明，长标题与技术名称可换行。
- 当前三张生产素材均为 1536 × 1024 WebP：`night-sea.webp` 为纯海景，`night-workbench.webp` 为保留完整底座的 RGBA 透明前景，`quiet-harbor.webp` 为海岸生活场景，并复用于明信片。合计约 620 KiB，来源和处理记录见同名 JSON sidecar。此前 `night-voyage.webp` 保留为旧版资产，不再用于当前首屏。
- 航船、器物和海岸使用栅格图像；SVG / 图标组件仅承担罗盘符号、箭头和操作等简单控件。文字、导航、按钮及档案均为 HTML，渐变遮罩只用于对比，不绘制复杂器物。生活图属于装饰性主题插画，不标称真实旅行照片。

### 动效、性能与可用性

- 首屏以海景和工作台的不同滚动幅度、鼠标轻视差、11 秒缓慢浮动建立景深；圆台越界是图像层与布局的真实叠压。实现是栅格分层结合 CSS 变换，不是实时 3D 船体或流体模拟。
- 项目切换采用短促翻页过渡，纸面随鼠标轻微倾斜；个人手记轻转，工作航线按阅读进度绘制，节点和条目进入视口后显现；海岸、地图与联系明信片使用不同幅度的滚动位移，明信片按钮切换正面与二维码。每种动效对应阅读、选档或联系行为，不另加粒子、自动跑马灯或持续旋转图标。
- `useAboutMotion` 通过 IntersectionObserver 管理可见场景，以按需 requestAnimationFrame 合并滚动与尺寸更新，统一读布局后写 CSS 变量；已显现内容保留可读，不反复重播。没有观察器或禁用动态时直接显示完整内容，离开页面清理监听、观察器和待执行帧。实现沿用 Vue、CSS 和原生浏览器 API，无新增运行时依赖。
- 章节导航提供暂停开关；首屏离屏暂停浮动，页面隐藏停止场景更新，触屏不启用鼠标视差。`prefers-reduced-motion` 关闭位移、旋转、过渡与平滑滚动，保留选择状态和功能。项目索引具备 tablist / tabpanel 语义、方向键和 Home / End 操作；纸面与深色背景分别提供可见焦点。简历使用原生 dialog，支持 ESC，关闭后卸载预览 iframe。
- 所在地复用延迟加载地图，显示于 185px 高的紧凑图面，卡片外层保留所在地标题，隐藏内部重复标题。高德深蓝样式和商标版权保留，缺少配置、坐标或加载失败时展示中文备用状态。当前验收结果与证据位置以 `design-qa.md` 为准，局部规则不代替实际验证。
