...
2026-09-24 按用户选择的图 2「夜航工作台」重设计，视觉依据为 `artifacts/about-design/2026-09-24/night/night.png`。本页采用固定夜色，属于用户确认的独立沉浸表面；全站其余页面与系统主题规则保持各自语义。导航仅增加夜色配色，不改变文字、入口、字体、图标、移动菜单和显隐行为。

### 布局与内容

- 首屏为左侧身份、主张与操作，右侧月夜航船、罗盘和日志。内容宽度最多 1384px，两侧留白为 5vw；首屏下方接项目经历与技术工具两栏，之后顺序展开个人介绍与指标、工作经历、兴趣与所在地、本站介绍、联系。
- 模块依据既有公开接口组织，项目、工作、指标、兴趣、技术标签及社交链接均支持可变数量和长文本。没有项目封面字段，不添加虚构封面；没有简历时不显示相应操作。首次请求失败展示重试，不以本地示例身份填充线上页面。
- 项目和经历使用连续记录面与细线分隔，不使用等大卡片墙或无依据的技能百分比。1150px 以下收紧比例，800px 以下改为单列，520px 以下继续收紧控件与留白；手机首屏先读文案，再看完整船体。

### 字体、颜色与图像

- 本页使用 `about-night` 环境、`about-workbench` 工作面、`about-paper` 主文字、`about-muted` 辅助字和 `about-brass` 操作反馈。`about-hairline` 用于连续分隔，黄铜保持少量。
- 主标题沿用全站 `--display-font`；正文使用 Noto Sans SC / Microsoft YaHei，日期与技术读数使用 IBM Plex Mono。中文正文通常为 15–20px，标题根据视口流动缩放，技术标签为 12px；较小英文栏目名仅为装饰性辅助，始终配有清晰中文标题。长主张用平衡换行，不能留下两字孤行。
- `web/public/about/night-voyage.webp` 是从选定参考生成的独立背景素材，1536 × 1024、约 202 KiB。界面文字、导航和按钮均为 HTML；桌面图像从顶部对齐以保留月亮，遮罩只负责文字对比。

### 动效、性能与可用性

- 单幅栅格场景采用 18 秒缓慢缩放、鼠标最多约 6px/4px 的轻视差和最多 35px 滚动漂移；这是图像空间动效，不是实时 3D 船体或水面模拟。
- 经历航线随阅读推进，章节短线随显现展开，控件和简历原生对话框有短过渡。一次只突出当前阅读区域，不增加粒子、自动跑马灯和重复循环图标。
- 提供暂停开关，首屏离开视口或页面隐藏时停止环境动画，触屏不使用鼠标视差；减少动态偏好关闭位移和显现动画。键盘焦点必须可见，简历支持 ESC 与焦点恢复，关闭即卸载预览 iframe。
- 地图接近视口才加载，高德使用官方深蓝样式；保留商标版权，缺少 key、坐标或加载失败时显示静态中文状态。离开页面清理观察器、事件监听与待执行帧。
