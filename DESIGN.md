...
展馆是航海日志系统中的独立沉浸表面，视觉基调为日光下的木质旗舰船舱：青绿色海面与船体环境、胡桃木拱肋、柚木甲板和黄铜饰件共同构成材料层次。这里的色彩和材质只约束 `/gallery`，不替换全站深海蓝、帆布、黄铜与珊瑚 token。

### 空间与容量

- 船体宽约 10m，内舱净通道约 6m；基础 6 个舱段按每段 3.4m 延展，随公开人物数量增长，最多支持 40 个展位。
- 运行时使用 `flagship/flagship.glb` 的 `CabinBay`、`BowDeck`、`SternDeck`、`ExhibitFrame` 结构单元；模型与 PBR 材质是已验证交付物，人物海报由接口数据注入。
- 展位与画布保持稳定尺寸，最多维护邻近 12 张海报纹理；性能目标应以具体验收环境记录，不外推为通用 60fps 保证。

### 入口与导航

- 首次进入显示低位入口层，保留展馆名称、入口标题和“进入展馆/人物档案”动作；进入漫游后入口层可用于暂停和继续。
- 桌面支持鼠标视角、WASD/方向键与 Pointer Lock 第一人称移动；E 或点击打开近距离展位，Escape 退出锁定。右上角使用半透明船舱小地图，显示船首/船尾、玩家方向和视野内展位。
- 小地图和准星属于展馆专属导航，不应抽象成全站通用组件。小屏或触摸设备直接降级为二维档案网格和详情对话框。

### 字体与状态

展馆沿用 `ZCOOL KuaiLe` 标题、`Noto Sans SC` 正文和 `IBM Plex Mono` 编号/距离读数；标题保持短行与正常字距，技术读数退居辅助层。青绿、胡桃木/柚木、黄铜与珊瑚色仅在展馆环境、材质高亮、焦点和近距提示中按语义使用，入口按钮、暂停态和档案对话框必须保持清晰对比与键盘焦点。

### 验收边界

浏览器证据保存在 `artifacts/gallery/qa/desktop-entry.png`、`desktop-tour.png` 和 `mobile.png`。这些截图证明入口、漫游和移动端降级路径；它们不证明严格 1:1 视觉一致性，也不代表所有硬件的性能结果。模型的连接件、老化微结构、舵台/船边轮廓及实时环境与参考图仍可能不同。
