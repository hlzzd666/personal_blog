...
序厅以独立舱室背景承载胡桃木、黄铜、宽舷窗和展柜氛围，标题、导航、搜索、全屏、人物展签、章节目录与罗盘均可独立交互。进入漫游后切换至 Three.js 实时封闭舱室；实时几何、光照和材质为功能重建，与概念图仍有差异，不属于逐像素或照片级复刻。
...
- 顶栏在序厅和漫游中固定；保留固定“序厅 / 全部馆藏”入口，其他分类及文案由后台配置，初始为“东海群像、伟大航路、新世界”。人物通过分类编号关联，未分类或分类隐藏时仍出现在全部馆藏；航海图志共用分类数据，经典瞬间使用全部公开人物。
...
- 保留 0～40 人容量、自动布展、碰撞、邻近最多 12 张海报纹理及资源释放。分类管理沿用管理员 Session/CSRF；迁移 `20260924_01` 新增分类表和人物归属，公开接口返回可见分类，管理接口支持分类维护与排序。图片上传与全局展位顺序沿用现有机制。
...
章节与人物图集为 AI 生成的非官方同人插画；序厅背景为真实场景截图。各 JSON 记录来源、描述性 brief 与 WebP 处理方式，其中旧图集的 `prompt` 字段是来源摘要，不代表完整原始生成提示词。管理员海报优先，缺失或加载失败时才使用对应馆藏插画；未识别人物使用中性编目海报。人物备用插画的姓名关联由 `web/src/gallery/curation.ts` 管理，不参与分类筛选；章节选择已有图集的插画编号，重新排序不会改变插画。替换图集时须同步裁切边界。
...
道具前使用 E 或点击打开纸面展签，展示介绍和外观识别点；模型署名、许可与修改说明集中由二维馆藏底部的素材来源入口查阅。关闭展签后恢复先前漫游状态；暂停时不显示重复提示。小地图用方点标示道具、圆点标示人物展位。二维馆藏底部同样提供六件道具的阅读入口，触屏和不支持 WebGL 的设备也可访问。
...
2026-09-24 分类配置验收：后端 54 项测试、前后台 lint/build 通过。本地数据库升级至 `20260924_01`，原人物资料及海报保留；实际后台新增、改名、分配人物、排序、隐藏与前台编号筛选通过，临时测试数据已恢复。桌面 1440px 与手机 390px 下验证了空分类、20 个分类和 80 字符名称，无页面横向溢出；线上尚未发布本次功能。
