...
import { BarChart, PieChart, TreemapChart, HeatmapChart } from "echarts/charts";
...
use([BarChart, PieChart, TreemapChart, HeatmapChart, GridComponent, LegendComponent, TooltipComponent, CalendarComponent, VisualMapComponent, GraphicComponent, AriaComponent, CanvasRenderer]);
export type ChartSelection = { name: string; data: unknown; value: unknown; seriesName?: string };
const props = withDefaults(defineProps<{ option: EChartsCoreOption; label: string; empty?: boolean; emptyText?: string }>(), { emptyText: "暂无文章" });
...
watch(() => props.option, (option) => {
  ...
  if (props.empty) root.value?.setAttribute("aria-label", `${props.label}，${props.emptyText}`);
});
...
<template>
  <div class="dashboard-chart-wrap">
    ...
    <span v-if="empty" class="dashboard-chart-empty">{{ emptyText }}</span>
  </div>
</template>
